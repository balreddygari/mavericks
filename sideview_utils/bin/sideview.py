...
moduleAttributeRe = re.compile('(\s+)<module ([^/>]+)(/)?>(.+)?')
annoyingCDATASpaceBugRe = re.compile('(.+)?(]]>)(\s+)(</param>)(.?)')
dashboardPanelRe = re.compile('panel_row(\d+)_col(\d+)(?:_grp(\d+))?')
...
def getAttributeValueForModule(app, view, moduleId, name) :
    ...
    if (not module) :
        ...
        raise KeyError, str("no module found with id " + moduleId)

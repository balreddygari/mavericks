...
if (typeof(Sideview)!="undefined") {
    ...
    Sideview.utils.declareCustomBehavior("insertNewPanelWithinEaiData", function(module) {
        module.onContextChange = function() {
            ...
            var doc = parser.parseFromString(eaiData, "text/xml");
            var rows = doc.getElementsByTagName("row");
            if (rows.length>0) {
                var firstChild = rows[0]
                alert("this is where I stopped.  Have to grab the very first node, no matter what tag it is (exclude whitspace? ) and then insertBefore that one.");

            }
            else {
                alert("existing dashboard has no rows");
            }
        }
    });
